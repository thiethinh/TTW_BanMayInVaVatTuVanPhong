...
public class AdminAccountServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        logger.debug("Nhận yêu cầu vào AdminAccountServlet với action: {}", action);

        if ("get-by-month".equals(action)) {
            try {
                int month = Integer.parseInt(request.getParameter("month"));
                int year = Integer.parseInt(request.getParameter("year"));
                logger.info("Yêu cầu AJAX: Lấy danh sách customer theo tháng {}/{}", month, year);
                ...
                logger.error("Lỗi parse dữ liệu month/year khi chạy hành động get-by-month: {}", e.getMessage());
            }
        }
        ...
        if (idParam != null && action != null) {
            try {
                ...
                logger.info("Thực hiện hành động '{}' cho User ID: {}", action, uid);

                if ("lock".equals(action)) {
                    userDAO.updateUserStatus(uid, false);
                    logger.info("Đã khóa tài khoản thành công cho User ID: {}", uid);
                } else if ("unlock".equals(action)) {
                    userDAO.updateUserStatus(uid, true);
                    logger.info("Đã mở khóa tài khoản thành công cho User ID: {}", uid);
                    ...
                    if ("admin".equals(newRole) || "user".equals(newRole) || "mod".equals(newRole)) {
                        ...
                        logger.info("Đã cập nhật role mới '{}' cho User ID: {}", newRole, uid);

                        if ("user".equals(newRole) || "admin".equals(newRole)) {
                            userDAO.updateModPermissions(uid, null);
                            logger.info("Đã cập nhật quyền của mod User ID: {} do chuyển sang role: {}", uid, newRole);
                        }
                    } else {
                        logger.warn("Cảnh báo: Role truyền vào không hợp lệ: '{}' cho User ID: {}", newRole, uid);
                    }
                } else if ("set-permission".equals(action)) {
                    String[] permissions = request.getParameterValues("permissions");
                    userDAO.updateModPermissions(uid, permissions);
                    logger.info("Đã cập nhật danh sách quyền mới {} cho Mod ID: {}", Arrays.toString(permissions), uid);
                }
            } catch (Exception e) {
                logger.error("Lỗi xảy ra khi thực hiện hành động '{}' trên User ID '{}': ", action, idParam, e);
            }
            ...
            logger.debug("Redirecting về trang quản lý với params: page={}, search={}, sort={}, role={}", pageStr, searchStr, sortStr, roleStr);
        ...
        try {
            ...
            logger.warn("Tham số page '{}' không hợp lệ, tự động quay về page 1.", request.getParameter("page"));
            page = 1;
        }

        logger.info("Tải danh sách tài khoản - Bộ lọc [Từ khóa: '{}', Trạng thái: '{}', Quyền: '{}', Trang: {}]",
        ...
        logger.debug("Tìm thấy tổng số {} users. Tổng số trang: {}", totalUsers, totalPages);
